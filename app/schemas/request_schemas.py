...
class Words(BaseModel):
    ...
    def validate_words(cls, words: List[str]) -> List[str]:
        ...
        if any(not word.strip() for word in words):
            raise ValueError("As palavras não podem estar vazias ou conter apenas espaços")

        return [word.strip() for word in words]

class WordsWithOrder(Words):
    order: SortOrder = Field(
        description="Ordem de classificação ('asc' para ascendente, 'desc' para descendente)",
        examples=["asc"]
    ) 